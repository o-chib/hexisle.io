...
export class CollisionObjectPool {
  ...
  private poolSize: number;
	...
	constructor(initialSize = 1000) {
		this.increase_percent = 0.50;
		this.minimum_percent_free = 0.10;
	...
	public getElement(): PoolCollisionObject {
		if (this.freeElements / this.poolArray.length <= this.minimum_percent_free) {
    ...
    const increaseSize = Math.round(this.increase_percent * this.poolArray.length);
